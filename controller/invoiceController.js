const Customer = require('../model/invoice');
...
exports.createCustomer = async (req, res) => {
  ...
  exports.getCustomers = async (req, res) => {
    try {
      ...
      const customers = await Customer.find(filter).populate('customer')
        .skip(skip)
        .limit(limit)
        .sort(sort);
  ...
  exports.getCustomerById = async (req, res) => {
    try {
      const customer = await Customer.findById(req.params.id).populate('customer').lean();
      if (!customer) {
        return res.status(404).json({ success: false, error: 'Customer not found' });
      }
      customer['text'] = numWords(customer.totalAmount) ;
